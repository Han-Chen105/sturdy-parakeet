...
int Seat_Perst_InsertBatch(seat_list_t list) {
	...
	long key = EntKey_Perst_GetNewKeys(SEAT_KEY_NAME, recCount);
	if (key <= 0) return 0;
	...
	while (p != list) {
		seat_t *d = &p->data;
		d->id = (int)key++;
		if (fwrite(d, sizeof(seat_t), 1, fp)) added++;
		p = p->next;
	}
...
int Seat_Perst_DeleteByID(int ID) {
	...
	if (NULL == fpSour) return 0;
	FILE *fpTarg = fopen(SEAT_DATA_FILE, "wb");
	if (NULL == fpTarg) { fclose(fpSour); return 0; }

	seat_t buf;
	int found = 0;
	while (!feof(fpSour)) {
		if (fread(&buf, sizeof(seat_t), 1, fpSour)) {
			if (buf.id == ID) { found = 1; continue; }
...
int Seat_Perst_DeleteAllByRoomID(int roomID) {
	...
	while (!feof(fpSour)) {
		if (fread(&buf, sizeof(seat_t), 1, fpSour)) {
			if (buf.roomID == roomID) { found = 1; continue; }
			fwrite(&buf, sizeof(seat_t), 1, fpTarg);
		}
	}
...
int Seat_Perst_SelectByID(int ID, seat_t *buf) {
	...
	while (!feof(fp)) {
		if (fread(&data, sizeof(seat_t), 1, fp)) {
			if (data.id == ID) { *buf = data; found = 1; break; }
		}
	}
...
int Seat_Perst_SelectAll(seat_list_t list) {
	...
	while (!feof(fp)) {
		if (fread(&data, sizeof(seat_t), 1, fp)) {
			...
			if (!newNode) { printf("Warning, Memory OverFlow!!!\n"); break; }
			newNode->data = data;
